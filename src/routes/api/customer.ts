...
router.post("/updateProfile", auth, async (req: Request, res: Response) => {
  ...
  try {
    let cust: ICustomer = await Customer.findOne({
    ...
    let cusPro: ICustomerProfile = await CustomerProfile.findOne({
      customerID: cust._id,
    });
...
router.post(
  "/login",
  [
    check("customerEmail", "Email is required").isEmail(),
    check("cusPassword", "Password is required").not().isEmpty(),
  ],
  async (req: Request, res: Response) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
